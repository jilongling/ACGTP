...
def _decode_context_tokens_for_flops(row: Dict[str, str], prefill_seq: float) -> Optional[float]:
    q_tokens = _row_float(row, "llm_decode_query_tokens")
    if q_tokens is None or q_tokens <= 0:
        return None
    ctx_tokens = _row_float(row, "llm_decode_context_tokens")
    # Older logs sometimes stored decode query count here (for example 6)
    # because the timing hook saw cache_position rather than the real KV cache.
    if ctx_tokens is not None and ctx_tokens > q_tokens:
        return ctx_tokens

    decode_calls = _row_float(row, "llm_decode_calls") or q_tokens
    effective_kv = (
        _row_float(row, "decode_effective_kv_tokens_mean")
        or _row_float(row, "internal_kv_cache_mean_seq_len")
    )
    if effective_kv is not None and effective_kv > 0:
        return float(effective_kv) * float(decode_calls)

    # Baseline fallback: full prefill cache grows by one token per decode call.
    # This is an estimate, but it is less misleading than using query-token count.
    return float(prefill_seq) * float(decode_calls) + float(decode_calls) * (float(decode_calls) + 1.0) / 2.0
...
def analytic_llm_flops(rows: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    for row in rows:
        ...
        q_tokens = _row_float(row, "llm_decode_query_tokens")
        ctx_tokens = _decode_context_tokens_for_flops(row, prefill_seq)
        decode = None
...
def compact_method(method_dir: Path, latency_scope: str = "llm_only") -> Dict[str, Any]:
    ...
    return {
        ...
        "latency_ms": {
            ...
            "llm_decode_context_tokens": first_mean(
                step_rows,
                ["llm_decode_context_tokens"],
                summary.get("mean_llm_decode_context_tokens"),
            ),
...
def method_core_summary(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "mean_llm_decode_context_tokens": l.get("llm_decode_context_tokens"),
...
def write_markdown(methods: List[Dict[str, Any]], baseline: Dict[str, Any], path: Path) -> None:
    ...
    lines.append("| Method | Vision | Projector | LLM Total | Prefill | Decode | Decode Ctx | Hook | Selector | Plan Cache Hit | Env Step | Primary | Raw Wall | Wall Ep Avg | Wall Step Records | Split Available |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for m in methods:
        l = m["latency_ms"]
        lines.append(
            ...
            f"{fmt_int(l.get('llm_decode_context_tokens'))} | "
