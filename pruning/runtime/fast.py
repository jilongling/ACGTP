"""Fast ACGTP runtime path for VisualTokenPruningHook."""
...
from ..signals.temporal import decide_acgtp_dynamic_budget
...
from ..internal.quota_config import build_internal_quota_config
...
class HookFastRuntimeMixin:
    ...
    @staticmethod
    def _cache_context_array_signature(value: Any, *, decimals: int = 6) -> Optional[str]:
        if value is None:
            return None
        try:
        ...
        flat = np.round(arr.reshape(-1), int(decimals))
        values = ",".join(f"{float(x):.6g}" for x in flat[:64])
        if flat.size > 64:
            values += ",..."
        return f"shape={tuple(arr.shape)};values={values}"

    def _depth_metadata_value(self, latest: Any, key: str) -> Any:
        metadata = getattr(latest, "depth_metadata", None)
        if isinstance(metadata, dict):
            return metadata.get(key)
        return getattr(metadata, key, None) if metadata is not None else None

    def _depth_source_key(self, latest: Any) -> Optional[str]:
        return self._cache_context_scalar(self._depth_metadata_value(latest, "source_key"))

    def _latency_plan_cache_context_key(
        self,
        latest: Any,
        *,
        depth_source_key: Optional[str],
        ...
        return (
            ...
            ("depth_conversion", self._cache_context_scalar(self._depth_metadata_value(latest, "conversion"))),
            ("depth_unit", self._cache_context_scalar(self._depth_metadata_value(latest, "depth_unit"))),
            ("depth_is_metric", self._cache_context_scalar(self._depth_metadata_value(latest, "depth_is_metric"))),
            ("camera_intrinsics", self._cache_context_array_signature(getattr(latest, "camera_intrinsics", None))),
            ("camera_transform_source", self._cache_context_scalar(transform_key)),
            ("camera_transform", self._cache_context_array_signature(T_robot_cam)),
            ("gripper_source", self._cache_context_scalar(gripper_key)),
        )
    ...
    def _build_internal_geometry_payload(
        ...
        *,
        ...
        quota_config = build_internal_quota_config(
            self.config,
    ...
    def _run_acgtp_fast_runtime(
        ...
        *,
        ...
        depth_source_key = self._depth_source_key(latest)
        plan_cache_context_key = self._latency_plan_cache_context_key(
            latest,
            depth_source_key=depth_source_key,
            gripper_key=gripper_key,
            transform_key=transform_key,
            T_robot_cam=T_robot_cam,
        )
        ...
        cached_plan, plan_cache_meta = self._lookup_latency_plan_cache(
            ...
            cache_context_key=plan_cache_context_key,
        )
        if cached_plan is not None and bool(plan_cache_meta.get("acgtp_latency_plan_cache_hit")):
            selection_meta = dict(cached_plan.get("selection_meta") or {})
            ...
            cached_robot_metrics = dict(cached_plan.get("robot_metrics") or {})
            ...
            return self._finalize_acgtp_fast_runtime(
                ...
                num_tokens=num_tokens,
                geometry_payload=cached_plan.get("geometry_payload"),
            )
