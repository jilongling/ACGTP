...
def compact_method(method_dir: Path) -> Dict[str, Any]:
    ...
    # whenever they exist.
    wall_ms = wall_global_ms if wall_global_ms is not None else wall_episode_ms
    model_ms = first_mean(
        step_rows,
    ...
    llm_total_ms = first_mean(step_rows, ["llm_forward_time_ms"], summary.get("mean_llm_forward_time_ms"))
    llm_prefill_ms = first_mean(
        step_rows,
    ...
    for group in groups.values():
        ...
        for m in group:
            ...
            if m["baseline_delta"].get("wall_speedup") is not None and m["baseline_delta"]["wall_speedup"] < 1.0:
                warnings.append("wall_clock_not_accelerated")
...
def method_core_summary(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    wall_ms = l["wall_step_primary"]
    control_hz = safe_div(1000.0, wall_ms)
    ...
    return {
        ...
        "hook_overhead_ratio_of_wall_step": safe_div(l["hook_total"], wall_ms),
        "mean_model_forward_time_ms": l["model_forward"],
        "mean_step_wall_time_ms": wall_ms,
        "mean_step_wall_time_ms_episode_avg": l["wall_step_episode_avg"],
        "mean_step_wall_time_ms_step_records": l["wall_step_global"],
        "wall_timing_scope": "inference_step_records_primary; episode_avg_preserved_separately",
        "control_frequency_hz": control_hz,
        "control_frequency_source": "1000 / mean_step_wall_time_ms_step_records",
        "control_frequency_scope": "inference control loop including model forward, hook, action postprocess, and env.step; excludes settling/no-op steps",
        "control_frequency_includes_env_step": True,
...
def flatten_row(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # baseline control frequency = method frequency / speedup, so speedup is wall speedup.
...
def write_markdown(methods: List[Dict[str, Any]], baseline: Dict[str, Any], path: Path) -> None:
    ...
    for m in methods:
        ...
        lines.append(
            ...
            f"wall {fmt_ratio(d['wall_speedup'])}; model {fmt_ratio(d['model_speedup'])}; "
    ...
    if main is not None:
        ...
        lines.append(
            f"| wall-clock decreases | {'YES' if (d.get('wall_speedup') or 0) > 1.0 else 'NO'} | "
            f"wall speed {fmt_ratio(d.get('wall_speedup'))} | not proven |"
        )
        lines.append(
    ...
    lines.append("| model/wall speedup | DERIVED_VALID | episode/step metrics + baseline comparison | yes, with confidence warnings | speedup < 1 is slowdown |")
    ...
    lines.append("| Method | Verdict | SR | SR Delta | Retain | Seq Retain | Wall | Model | LLM Saved | Hook | Net LLM-Hook | Eff Attn Speed | Warnings |")
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for m in methods:
        d = m["baseline_delta"]
        t = m["tokens"]
        ...
        lines.append(
            ...
            f"{fmt_ratio(d['model_speedup'])} | {fmt_ms(d['llm_saved_ms'])} | "
    ...
    lines.append("| Method | Paired N | Paired Wall | Paired Model | Paired LLM Saved | Paired Success Delta | Confidence |")
    lines.append("|---|---:|---:|---:|---:|---:|---|")
    for m in methods:
        ...
        lines.append(
            ...
            f"{fmt_ratio(paired.get('paired_wall_speedup'))} | "
    ...
    lines.append("| Method | Strict Paired N | Strict Wall | Strict Model | Strict LLM Saved | Strict Success Delta |")
    lines.append("|---|---:|---:|---:|---:|---:|")
    for m in methods:
        ...
        lines.append(
            ...
            f"{fmt_ratio(strict_paired.get('paired_wall_speedup'))} | "
    ...
    lines.append("| Method | Vision | Projector | LLM Total | Prefill | Decode | Hook | Selector | Plan Cache Hit | Env Step | Wall Primary | Wall Ep Avg | Wall Step Records | Split Available |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for m in methods:
        l = m["latency_ms"]
        lines.append(
            ...
            f"{fmt_ms(l['wall_step_primary'])} | {fmt_ms(l['wall_step_episode_avg'])} | {fmt_ms(l['wall_step_global'])} | "
    ...
    lines.append("`control_frequency_hz` is computed as 1000 / mean inference-step wall time from `step_metrics.csv`; it includes model forward, pruning hook, action postprocess, and `env.step`, and excludes settling/no-op steps.\n")
    ...
    lines.append("- Wall-clock speedup uses `step_metrics.total_step_wall_time_ms` as the primary inference-step wall time; episode averages may include settling/no-op steps.")
    ...
    lines.append("- Do not claim stable end-to-end acceleration unless quality is preserved and wall speedup is consistently above 1.0 across larger paired runs.\n")
...
def build(root: Path, selected: Optional[List[str]], baseline_label: Optional[str], prefix: str) -> Dict[str, Any]:
    dirs = method_dirs(root, selected)
    if not dirs:
        raise SystemExit(f"No method directories found under {root}")
    methods = [compact_method(d) for d in dirs]
...
def main() -> None:
    ...
    report = build(args.root.resolve(), args.method, args.baseline, args.prefix)
